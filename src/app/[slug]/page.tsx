...
export async function generateMetadata({ params }: CityPageProps) {
  ...
  // Build a keyword-rich, unique title â€” use metaTitle from DB if set, otherwise generate
  const title = c.metaTitle ||
    `Carpet, Window & Upholstery Cleaning in ${c.name}, ${c.state} | Tropical Breeze RFâ„¢`;

  // Build a unique description â€” use metaDescription from DB if set, otherwise generate from local data
  const description = c.metaDescription || (() => {
    ...
    return `${hook}Tropical Breeze RFâ„¢ delivers residue-free carpet, window, upholstery, tile & EZ Breeze cleaning in ${c.name}, ${c.state}. ${rentals}${golf}Prochem truckmount + Rotovac Powerwand 360Â°. Call 443-856-3244.`;
  })();
...
export default async function CityPage({ params }: CityPageProps) {
  ...
  const services = [
    { icon: "ðŸ§¼", name: "Carpet Cleaning", price: "$99 first room", slug: "carpet-cleaning", desc: "RF99â„¢ process with Prochem truckmount and Rotovac Powerwand 360Â°. Dry in 4â€“6 hours.", active: city.offersCarpet },
    { icon: "ðŸ›‹ï¸", name: "Upholstery Cleaning", price: "From $50", slug: "upholstery", desc: "Fiber-specific RFâ„¢ process for every sofa, chair, and sectional. Pet odor enzyme treatment available.", active: city.offersUpholstery },
    { icon: "â¬œ", name: "Tile and Grout", price: "$125 per room", slug: "tile-grout", desc: "Up to 1,200+ PSI rotary extraction. Orange sprinkler stains are our Eastern Shore specialty.", active: city.offersTileGrout },
    { icon: "ðŸªµ", name: "Hardwood Floors", price: "$1.00 per sq ft", slug: "hardwood", desc: "Low-moisture RFâ„¢ process safe for all finishes. Free inspection before every job.", active: city.offersHardwood },
    { icon: "ðŸªŸ", name: "Window Cleaning", price: "$13 per window", slug: "window-cleaning", desc: "Pure water process removes salt-air mineral deposits. Inside, outside, screens, tracks, sills.", active: city.offersWindow },
    { icon: "ðŸŒ´", name: "EZ Breeze Cleaning", price: "Free assessment", slug: "ez-breeze", desc: "Vinyl-safe solutions only. Never ammonia-based. Restores your view streak-free.", active: city.offersEzBreeze },
    { icon: "ðŸª„", name: "Area Rug Cleaning", price: "From $75", slug: "area-rugs", desc: "Pickup and delivery included. RFâ„¢ process safe for all rug types and fibers.", active: city.offersAreaRugs },
  ].filter(s => s.active);

  const faqs = [
    {
      q: `How much does carpet cleaning cost in ${city.name}?`,
      a: `Our RF99â„¢ process starts at $99 for the first room (up to 200 sq ft) and $50 for each additional room in ${city.name}. This includes our Prochem truckmount hot water extraction and Rotovac Powerwand 360Â° at 300â€“500 PSI. Dry in 4â€“6 hours.`,
    },
    {
      q: `What makes RFâ„¢ cleaning different from other carpet cleaners in ${city.name}?`,
      a: `Traditional cleaning companies in the ${city.state} area leave soap residue in carpet fibers that re-attracts dirt within 2 weeks. Our RFâ„¢ (Residue-Free) process uses a neutralization rinse that removes every cleaning molecule after extraction. Your carpets stay cleaner up to 3Ã— longer.`,
    },
    {
      q: `Do you serve all of ${city.name} and surrounding areas?`,
      a: `Yes â€” we serve all of ${city.name} and the surrounding ${city.region || city.state} area including ${nearbyList.slice(0, 3).join(', ')}${nearbyList.length > 3 ? ' and more' : ''}. Call 443-856-3244 to confirm availability for your specific address.`,
    },
    {
      q: `What equipment do you use in ${city.name}?`,
      a: `We use a professional Prochem truckmount hot water extraction system with a Rotovac Powerwand 360Â° for carpet cleaning at 300â€“500 PSI. For tile and grout we use up to 1,200+ PSI rotary extraction. This is professional grade equipment, not consumer rental machines.`,
    },
    {
      q: `How long does carpet cleaning take in ${city.name}?`,
      a: `Most rooms take 20â€“40 minutes. A full home is typically 2â€“4 hours depending on size and condition. Carpets are walk-ready in 4â€“6 hours with our RFâ„¢ process â€” not 24â€“48 hours like traditional cleaning.`,
    },
    {
      q: `Is RFâ„¢ cleaning safe for pets and children in ${city.name}?`,
      a: `Completely safe. The RFâ„¢ process is specifically designed to remove all cleaning agents from fibers. There is nothing left behind for pets or children to contact. Safe immediately after drying.`,
    },
  ];
  ...
  // LocalBusiness schema with GeoCoordinates â€” unique per city
  const localBusinessSchema = {
    "@context": "https://schema.org",
    "@type": "LocalBusiness",
    "@id": `https://tropicalbreezerf.com/${citySlug}#business`,
    name: "Tropical Breeze RFâ„¢",
    url: "https://tropicalbreezerf.com",
    telephone: "+1-443-856-3244",
    image: "https://tropicalbreezerf.com/logo.png",
    description: `Residue-free cleaning in ${city.name}, ${city.state}. ${activeServiceNames.join(', ')}. Prochem truckmount + Rotovac Powerwand 360Â°.`,
    address: {
      "@type": "PostalAddress",
    ...
    hasOfferCatalog: {
      "@type": "OfferCatalog",
      name: `RFâ„¢ Cleaning Services in ${city.name}`,
  ...
  // FAQ schema â€” unique per city because questions include city name
  const faqSchema = {
    "@context": "https://schema.org",
    "@type": "FAQPage",
  ...
  return (
    <main className="min-h-screen overflow-x-hidden">
      ...
      <section className="relative py-32 px-6 bg-gradient-to-br from-[#0a1628] via-[#004d5a] to-[#006978] overflow-hidden">
        ...
        <div className="max-w-4xl mx-auto relative z-10">
          <div className="inline-flex items-center gap-2 bg-teal-500 bg-opacity-20 border border-teal-400 border-opacity-30 text-teal-300 text-xs font-bold tracking-widest uppercase px-4 py-2 rounded-full mb-8">
            ...
            Tropical Breeze RFâ„¢ Â· {city.state} Â· {city.region || "Eastern Shore"}
          </div>
          <h1 className="font-black text-white leading-none mb-6" style={{fontSize:"clamp(40px,7vw,88px)"}}>
            {city.h1Headline || `RFâ„¢ CLEANING IN ${city.name.toUpperCase()}, ${city.state}`}
          </h1>
          {city.localHook && (
          ...
          <p className="text-lg text-sky-100 leading-relaxed max-w-2xl mb-10">
            {city.localChallenge || `Professional residue-free cleaning in ${city.name}, ${city.state}. Prochem truckmount, Rotovac Powerwand 360Â°, zero residue left behind. Serving all of ${city.region || city.state}.`}
          </p>
          <div className="flex flex-wrap gap-4 mb-12">
            <Link href="/booking" className="bg-gradient-to-r from-orange-500 to-orange-600 text-white font-black text-lg px-10 py-5 rounded-full shadow-2xl hover:-translate-y-1 transition-all">
              ðŸŒ´ Book RFâ„¢ Service in {city.name}
            </Link>
      ...
      {/* FEATURED REVIEW â€” unique per city if available */}
      {city.featuredReview && (
        <section className="py-12 px-6 bg-orange-500">
          <div className="max-w-3xl mx-auto text-center">
            <div className="text-white text-2xl mb-3">â˜…â˜…â˜…â˜…â˜…</div>
            <p className="text-white font-bold text-xl italic mb-3">&ldquo;{city.featuredReview}&rdquo;</p>
            {city.featuredReviewer && (
              <p className="text-orange-100 text-sm font-semibold">â€” {city.featuredReviewer}, {city.name}</p>
            )}
          </div>
        </section>
      )}
      ...
      <section className="py-24 px-6 bg-white">
        <div className="max-w-6xl mx-auto">
          <div className="grid lg:grid-cols-2 gap-16 items-start">
            <div>
              <span className="text-orange-500 text-xs font-bold tracking-widest uppercase block mb-4">{city.name}, {city.state} Â· Local Expertise</span>
              ...
              {city.waterQualityNote && (
                <div className="bg-teal-50 border-l-4 border-teal-500 rounded-r-xl p-5 mb-6">
                  <div className="text-teal-700 font-bold text-sm mb-1">ðŸ’§ Water Quality Note</div>
              ...
              {city.hasGolfCommunity && (
                <div className="bg-green-50 border-l-4 border-green-500 rounded-r-xl p-5 mb-6">
                  <div className="text-green-700 font-bold text-sm mb-1">â›³ Golf Community Specialist</div>
              ...
              {city.hasVacationRentals && (
                <div className="bg-blue-50 border-l-4 border-blue-500 rounded-r-xl p-5 mb-6">
                  <div className="text-blue-700 font-bold text-sm mb-1">ðŸ–ï¸ Vacation Rental Specialist</div>
                  <p className="text-blue-600 text-sm leading-relaxed">RFâ„¢ 4â€“6 hour dry time means same-day turnaround between guests in {city.name}. Never lose a rental night to wet carpets.</p>
                </div>
              )}
              <p className="text-gray-600 text-lg leading-relaxed mb-6">
                The RFâ„¢ process was specifically developed for Eastern Shore conditions. Our Prochem truckmount system with Rotovac Powerwand 360Â° delivers 200Â°F+ heat extraction followed by a neutralizing rinse that removes every cleaning molecule â€” leaving surfaces genuinely clean, not just visually clean.
              </p>
              <div className="space-y-3">
                {[
                  `Serving all of ${city.name} and ${city.region || city.state}`,
                  "Prochem truckmount + Rotovac Powerwand 360Â° on every job",
                  "RFâ„¢ process â€” stays cleaner 3Ã— longer than traditional cleaning",
                  "Licensed, insured, locally operated from Salisbury MD",
                  "365+ five-star reviews across the Eastern Shore",
                ].map((item) => (
                  <div key={item} className="flex items-start gap-3">
                    <span className="text-teal-500 font-bold flex-shrink-0 mt-1">âœ“</span>
            ...
            <div className="space-y-4">
              <div className="bg-gray-50 rounded-2xl p-6 border border-gray-100">
                <h3 className="font-black text-[#0a1628] text-lg mb-4">Why RFâ„¢ Works Better in {city.name}</h3>
                <div className="space-y-4">
                  {[
                    { icon: "ðŸŒŠ", title: "Salt Air Mineral Deposits", desc: "Coastal proximity means windows and tile accumulate mineral deposits faster. Our pure water process and high-PSI extraction dissolve them completely." },
                    { icon: "ðŸ’§", title: "Iron-Rich Groundwater", desc: "Eastern Shore well water leaves orange iron staining on tile grout. Our mineral-specific treatment is the only reliable solution." },
                    { icon: "â˜ï¸", title: "High Coastal Humidity", desc: "Humidity accelerates mold growth in grout and re-soiling in carpet. RFâ„¢ residue-free process eliminates the sticky surface that attracts moisture-borne contamination." },
                    { icon: "ðŸ–ï¸", title: "Vacation Rental Turnover", desc: "RFâ„¢ 4â€“6 hour dry time means same-day turnaround between guests. Never lose a rental night to wet carpets." },
                  ].map((item) => (
              ...
              {city.housingType && (
                <div className="bg-teal-50 rounded-2xl p-5 border border-teal-100">
                  <div className="text-teal-700 font-bold text-sm mb-1">ðŸ  Primary Housing in {city.name}</div>
              ...
              {city.seasonalPeak && (
                <div className="bg-orange-50 rounded-2xl p-5 border border-orange-100">
                  <div className="text-orange-700 font-bold text-sm mb-1">ðŸ“… Peak Cleaning Season</div>
                  <p className="text-orange-600 text-sm">{city.seasonalPeak} â€” book early for best availability.</p>
                </div>
              )}
            </div>
          </div>
      ...
      <section className="py-16 px-6 bg-orange-500">
        <div className="max-w-4xl mx-auto flex flex-wrap items-center justify-between gap-6">
          <div>
            <p className="text-orange-100 text-xs font-bold tracking-widest uppercase mb-2">ðŸŒ¸ Spring Offer</p>
            <h2 className="font-black text-white text-4xl">RF99â„¢ â€” First Room $99</h2>
      ...
      <section className="py-24 px-6 bg-gray-50">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-16">
            <span className="text-orange-500 text-xs font-bold tracking-widest uppercase block mb-4">RFâ„¢ Services in {city.name}</span>
            ...
            <p className="text-gray-500 mt-4 text-lg">Every service performed to the RFâ„¢ residue-free standard in {city.name}, {city.state}.</p>
          </div>
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid md:grid-cols-2 gap-8">
            <div className="bg-gray-50 rounded-2xl p-8 border border-gray-100">
              <h3 className="font-black text-[#0a1628] text-xl mb-3">Rotovac Powerwand 360Â°</h3>
              <p className="text-gray-600 leading-relaxed mb-4">Standard wands only clean in two directions. Our Rotovac Powerwand uses dual-rotary heads to deep-clean every fiber from 360Â°, removing significantly more soil than traditional methods at 300â€“500 PSI for carpet.</p>
              <div className="bg-teal-50 rounded-xl p-4 border border-teal-100">
                <div className="text-teal-700 text-sm font-bold">Carpet PSI: 300â€“500</div>
                <div className="text-teal-600 text-xs mt-1">Appropriate pressure for carpet fibers â€” not damaging like high-pressure systems</div>
              </div>
            </div>
            <div className="bg-gray-50 rounded-2xl p-8 border border-gray-100">
              <h3 className="font-black text-[#0a1628] text-xl mb-3">Prochem Truckmount</h3>
              <p className="text-gray-600 leading-relaxed mb-4">The gold standard in hot water extraction. We deliver 200Â°F+ heat directly from our truck to your door, instantly sanitizing and killing bacteria on contact. Combined with our RFâ„¢ neutralization process for zero residue results.</p>
              <div className="bg-teal-50 rounded-xl p-4 border border-teal-100">
      ...
      {nearbyList.length > 0 && (
        <section className="py-16 px-6 bg-white">
          <div className="max-w-4xl mx-auto text-center">
            ...
            <p className="text-gray-500 mb-8">RFâ„¢ cleaning available throughout {city.region || city.state} and the Eastern Shore.</p>
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid md:grid-cols-4 gap-6">
            {[
              ...
              { stat: "3Ã—", label: "Longer clean with RFâ„¢ vs traditional methods", source: "Tropical Breeze RFâ„¢" },
      ...
      <section className="py-24 px-6 bg-gradient-to-br from-[#0a1628] to-[#004d5a] text-center">
        <div className="max-w-3xl mx-auto">
          <h2 className="font-black text-white leading-none mb-6" style={{fontSize:"clamp(36px,5vw,72px)"}}>
            READY FOR RFâ„¢ CLEAN<br /><span className="text-orange-400">IN {city.name.toUpperCase()}?</span>
          </h2>
          <p className="text-sky-200 text-xl mb-10">Residue Doesn&apos;t Survive Hereâ„¢ Â· Serving {city.name} and all of {city.region || city.state}</p>
          <div className="flex flex-wrap gap-4 justify-center">
            <Link href="/booking" className="bg-gradient-to-r from-orange-500 to-orange-600 text-white font-black text-xl px-12 py-6 rounded-full shadow-2xl hover:-translate-y-1 transition-all">
              ðŸŒ´ Book Online â€” Get Instant Quote
            </Link>
